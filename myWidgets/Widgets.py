...
class QCustomSlideMenu(QWidget):
    ...
    def openButton(self, **values):
        ...
        if "buttonName" in values:
            ...
            self.openMenuButton(self.targetBtn)

    def openMenuButton(self, buttonObject):
        buttonObject.clicked.connect(lambda: self.expandMenu())

    def closeButton(self, **values):
        if not hasattr(self, "targetBtn") and not "buttonName" in values:
            raise Exception("No button specified for this widget, please specify the QPushButton object")
            return
        ...
        if "buttonName" in values:
            ...
            self.closeMenuButton(self.targetBtn)
    
    def closeMenuButton(self, buttonObject):
        buttonObject.clicked.connect(lambda: self.collapseMenu())
    ########################################################################
    # Slide menu function
    ########################################################################
    def slideMenu(self):
        ...
        for QCustomSlideMenu in data['QCustomSlideMenu']:
            if "name" in QCustomSlideMenu and len(str(QCustomSlideMenu["name"])) > 0:
                if hasattr(self.ui, str(QCustomSlideMenu["name"])):
                    ...
                    if "openButton" in QCustomSlideMenu:
                        for toggleButton in QCustomSlideMenu["openButton"]:
                            if "buttonName" in toggleButton and len(str(toggleButton["buttonName"])) > 0:
                                if hasattr(self.ui, str(toggleButton["buttonName"])):
                    ...
                    if "closeButton" in QCustomSlideMenu:
                        for toggleButton in QCustomSlideMenu["closeButton"]:
                            if "buttonName" in toggleButton and len(str(toggleButton["buttonName"])) > 0:
                                if hasattr(self.ui, str(toggleButton["buttonName"])):
